...
BASE = Path("Chapter 4")
VAE_DIR = BASE / "vae" / "data"
# DATA_DIR = BASE / "data"
FWD, LAT  = VAE_DIR/"vae_reconstructed_fwd_rates.csv", VAE_DIR/"vae_fwd_latent_space.csv"
HIST_FWD  = VAE_DIR/"GLC_fwd_curve_raw.csv"
RP = VAE_DIR/"risk_premium_values.csv"
...
def load_risk_premium(csv_path, taus):
    """Read tenor,risk_premium and align to taus via linear interp (flat outside)."""
    df = pd.read_csv(csv_path)
    t_num = np.array([parse_tenor(x) for x in df["tenor"]], float)
    risk_prem = df["risk_premium"].astype(float).to_numpy()
    index = np.argsort(t_num) 
    t_num, risk_prem = t_num[index], risk_prem[index]
    return np.interp(taus, t_num, risk_prem, left=risk_prem[0], right=risk_prem[-1])

def simulate_path(F_hat, Sigma, taus, rng_seed=RNG_SEED, risk_prem=None):
    """
    Euler–Maruyama with explicit Musiela shift:
        f_{t+dt}(τ) = f_t(τ) + [ α_HJM(t,τ) + ∂f/∂τ (t,τ) + RP(τ) ] dt + Σ(t,τ) dW_t
    where α_HJM is computed from Sigma(t,·), and RP(τ) is a tenor-aligned
    risk premium vector (length N, per annum, in decimals).
    """
    rng = np.random.default_rng(rng_seed)
    T, N, P = Sigma.shape
    path = np.empty((T, N), float); path[0] = F_hat[0]
    Alpha = np.empty((T-1, N), float)

    # keep the caller-provided risk premium (or fall back to zeros)
    rp = np.zeros(N, float) if risk_prem is None else np.asarray(risk_prem, float).reshape(N)

    for t in range(1, T):
        fprev = path[t-1]
        Sigma_tm1 = Sigma[t-1]                            # (N, P)
        alpha_hjm = drift_computation(Sigma_tm1, taus, POLY_DEG)  # (N,)
        dfdtau    = np.gradient(fprev, taus)              # (N,)
        dW        = rng.normal(size=P) * np.sqrt(DT)      # (P,)
        diffusion = Sigma_tm1 @ dW                        # (N,)
        drift     = alpha_hjm + dfdtau + rp               # include RP(τ)

        Alpha[t-1] = alpha_hjm
        path[t]    = fprev + drift * DT + diffusion
...
if __name__=="__main__":
    ...
    risk_prem = load_risk_premium(RP, taus)

    # 4) Simulate with calibrated Sigma (unchanged logic)
    sim_full, Alpha = simulate_path(F_hat, Sigma, taus, risk_prem=risk_prem)
